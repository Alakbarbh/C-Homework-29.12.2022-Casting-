using Homework_prac.Casting29._12._2022.Models;

//Salmon salmon1 = new Salmon();

//Eagle eagle1 = new Eagle();

//Animal animal1 = eagle1;

//animal1.Sound();

//eagle1.Test();

//animal1.Test();

//Console.WriteLine(animal1.Speed);

//Animal animal2 = salmon1;

//Bird bird = new Bird();

//Animal animal3 = new Salmon();


//salmon1.Sound();

//eagle1.Sound();

//Animal animal1 = new Animal();
//animal1.Sound();

//Bird bird1 =new Bird();
//bird1.Sound();

//Bird bird= new Bird();
//Fish fish= new Fish();

//Eagle eagle1= new Eagle();

//Animal animal1 = new Animal();

//Fish fish1 = new Fish();

//Animal animal2 = new Animal();

//Bird bird1 = new Bird();

//Eagle eagle3 =(Eagle)animal2;

//Bird bird2 = new Bird();

//Eagle eagle = (Eagle)bird2;

//Animal[] animals = { eagle1, animal1, fish1, animal2, bird1, eagle3 };

//foreach (Animal item in animals)
//{
//    item.Name = "AAA";
//    Console.WriteLine(item.Name);
//}


//string str = "Eycahan";
//object obj1 = str;
//string name = obj1.ToString();


//int num1 = 55;
//object obj1 = num1;
//int num2 = (int)obj1;


//string str = "55";
//object obj1 = str;
//int num3 = (int)obj1;
//var result = int.Parse(str);


//bool isMarried = false;
//object obj1 = isMarried;
//bool result = (bool)obj1;
//Console.WriteLine(result);

//byte num1 = 22;

//int num1 = 1200;

//byte num2 = (byte)num1;

//Console.WriteLine(num2);

//byte num3 = 55;

//int num4 = num3;

//Console.WriteLine(num4);

Bird bird= new Bird();

Fish fish= new Fish();

Animal animal1 = new Animal();

Animal animal2 = new Animal();

object[] objects = { 1, 2, "Salam", bird, fish, "Necesen", animal1, animal2 };

//foreach (var item in objects)
//{
//	if (item is Bird)
//	{
//		((Bird)item).Name = "Eycahan";
//		Console.WriteLine(((Bird)item).Name);
//	}
//	else
//	{
//		Console.WriteLine(item);
//	}
//}

//foreach (var item in objects)
//{
//	if (item is Animal a)
//	{
//		a.Name = "Eycahan";
//		Console.WriteLine(a.Name);
//	}
//	else
//	{
//		Console.WriteLine(item);
//	}
//}

//foreach (var item in objects)
//{
//    Animal animal = item as Animal;
//	if (animal != null)
//	{
//		//animal.Name = "Eycahan";
//		//Console.WriteLine(animal.Name);

//		Console.WriteLine("Ede bilmedi");
//	}
//}
